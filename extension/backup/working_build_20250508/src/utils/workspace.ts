...
export async function duplicateWorkspace(workspace: Workspace, newName?: string, newColor?: string): Promise<Workspace> {
  const workspaces = await getWorkspaces();
  ...
  await chrome.storage.local.set({
    workspaces: [...workspaces, duplicatedWorkspace]
  });

  return duplicatedWorkspace;
}

export async function mergeWorkspaces(workspaceIds: string[], newName: string, newColor?: string): Promise<Workspace> {
  const workspaces = await getWorkspaces();
  const workspacesToMerge = workspaces.filter(w => workspaceIds.includes(w.id));
  
  if (workspacesToMerge.length === 0) {
    throw new Error('No workspaces to merge');
  }

  const mergedTabs = workspacesToMerge.flatMap(w => w.tabs);
  const uniqueTabs = Array.from(new Map(mergedTabs.map(tab => [tab.url, tab])).values());

  const mergedWorkspace: Workspace = {
    id: Date.now().toString(),
    name: newName,
    tabs: uniqueTabs,
    color: newColor || workspacesToMerge[0].color,
    createdAt: Date.now(),
    updatedAt: Date.now()
  };
